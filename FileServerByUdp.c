...
int main(){			
	...
	while (!_kbhit()){	
		...
		int alen = sizeof(clsin);						 
		char filename[FILELEN + 1] = {0};
		int recvlen = recvfrom(msock, filename, FILELEN + 1, 0, (struct sockaddr *)&clsin, &alen);//接收文件名
		buf[recvlen] = '\0';
		printf("正在接收文件：%s\n", filename);
		refilename(folder, filename); //为重名文件添加序号

		char msg[BUFLEN + 1];
		recvlen = recvfrom(msock, msg, BUFLEN + 1, 0, (struct sockaddr *)&clsin, &alen); //接收文件时间和大小
		msg[recvlen] = '\0';
		printf("%s\n",msg);
		...
		int length = 0;
		while((length = recvfrom(msock, buf, BUFLEN + 1, 0, (struct sockaddr *)&clsin, &alen)) > 0) {// 循环接收数据写入文件 
			Sleep(1);
		...
		sendto(msock, buf, BUFLEN + 1, 0, (struct sockaddr *)&clsin, alen);  
		closesocket(msock);					  
	}
	WSACleanup();		
	printf("服务器运行结束\n");
